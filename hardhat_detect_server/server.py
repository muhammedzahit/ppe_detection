...
def predict_hardhat(image_path = None, image_data = None):
    ...
    if SAVE_RESULTS:
        if image_path:
            plot_results(results, image_path=image_path, labels=labels, result_name = 'hardhat_pred_' + str(counter) + '.jpg')
        else:
            plot_results(results, image_data=image_data, labels=labels, result_name = 'hardhat_pred_' + str(counter) + '.jpg')
    counter += 1
